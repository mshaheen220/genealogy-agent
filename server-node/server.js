...
{
  "type": "sql" | "vector",
  "query": "The exact SQL query to run, OR the search keywords to use for the vector database"
}`),
...
Assume the SQL results correctly answer the user's question. Formulate a polite, conversational response using ONLY these results. If it's a list of names, list them out naturally. If it's a count, state the number.`),
      ...
      let results = rawResults.map(r => {
        ...
        queryWords.forEach(w => {
          ...
          if (sourceName.includes(w)) keywordScore += 0.75; // HUGE boost if the name is the actual profile subject
        });
...
To ensure accuracy, you MUST wrap your internal reasoning in <thinking> tags before providing your final conversational answer. 
...
FAMILY TREE DATA:\n${contextText}`),
      ...
      const cleanContent = aiResponse.content.replace(/<thinking>[\s\S]*?<\/thinking>\n*/gi, '').trim();
      const uiAnswer = `${cleanContent}<div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(0,0,0,0.05); font-size: 0.85em; color: var(--text-muted);">📚 <b>Sources:</b> ${topSources.replace(/\.md/g, '').replace(/_/g, ' ')}</div>`;
      socket.emit("answer", uiAnswer);
